...
export type LocalDiary = { id: string; date: string; frontId: string; service: string; summary: string; occurrence: string; weather: string; workforce: number; hours: number; production: number; evidenceName?: string };
...
const seed: LocalProject = {
  name: "Jardim Planalto", location: "São Paulo", status: "Em execução", description: "Loteamento em 2 etapas · cerca de 500 lotes",
  fronts: [
  ...
  actions: ["Executar dreno lateral abaixo da tubulação", "Mobilizar sistema de bombeamento", "Realizar bombeamento e rebaixamento", "Repor material com escavadeira", "Espalhar e regularizar com trator", "Aplicar camada drenante de rachinha", "Inspecionar e liberar o fundo", "Retomar assentamento da tubulação", "Registrar evidência fotográfica do trecho"].map((title, index) => ({ id: `act-${index + 1}`, title, owner: index === 0 ? "Equipe Remígio" : "Não informado", due: "A definir", priority: "Crítica", done: false })),
  diaries: [],
};
...
export function useLocalProject() {
  ...
  const addDiary = useCallback((entry: Omit<LocalDiary, "id">) => update((current) => ({ ...current, diaries: [{ ...entry, id: `diary-${Date.now()}` }, ...current.diaries], fronts: current.fronts.map((front) => {
    if (front.id !== entry.frontId) return front;
    const executed = front.executed + entry.production;
    return { ...front, executed, progress: Math.min(100, Math.round((executed / Math.max(front.planned, 1)) * 100)), status: entry.production > 0 ? "Em execução" : front.status };
  }) })), [update]);
