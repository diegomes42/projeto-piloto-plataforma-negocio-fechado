...
export default function PreferencesPage() {
  ...
  const save = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!name.trim() || !location.trim() || !description.trim()) {
      toast.error("Informe o nome, a localização e uma descrição curta da obra.");
      return;
    }
  ...
  return <div className="min-h-screen bg-[#ececea] px-4 py-8 sm:px-8 lg:px-12"><div className="mx-auto max-w-4xl">
    ...
    <div className="mt-8 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
      ...
      <div className="space-y-5"><Card className="rounded-none border-0 bg-[#202321] text-white shadow-[5px_5px_0_#c7c9c2]"><CardContent className="p-6"><Settings2 className="h-5 w-5 text-[#b8d36a]" /><Kicker>Aplicativo</Kicker><h2 className="mt-3 text-2xl font-black uppercase leading-tight">Módulo: Obras</h2><p className="mt-2 text-sm text-white/65">Operação de campo</p><p className="mt-5 text-xs leading-5 text-white/55">Este nome identifica o módulo de obras. Os dados da obra continuam independentes e podem ser atualizados ao longo do uso.</p></CardContent></Card><Card className="rounded-none border-0 border-l-4 border-l-[#d89b45] bg-[#f6f6f3] shadow-[5px_5px_0_#d0d1cb]"><CardContent className="p-6"><Download className="h-5 w-5 text-[#b67b2c]" /><Kicker>Seus dados</Kicker><p className="mt-3 text-sm leading-6 text-[#70756e]">A plataforma continua guardando seus dados neste navegador. Faça um backup antes de trocar de aparelho ou limpar o navegador.</p><Button type="button" variant="outline" onClick={() => navigate("/dados")} className="mt-4 border-black/15 bg-white">Abrir backup</Button></CardContent></Card></div>
    </div>
  </div></div>;
}
