...
function readPhoto(file: File): Promise<{ dataUrl: string; type: string }> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      ...
      image.onload = () => {
        ...
        if (!context) {
          resolve({ dataUrl: String(reader.result), type: file.type });
          return;
        }
        context.drawImage(image, 0, 0, canvas.width, canvas.height);
        resolve({ dataUrl: canvas.toDataURL("image/jpeg", 0.8), type: "image/jpeg" });
      };
...
export default function OperationalPage({ mode }: Props) {
  const { project, addDiary, addEvent, setEventStatus, addAction, toggleAction, addService } = useLocalProject();
  ...
  const saveDiary = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (evidence) {
      try {
        const photo = await readPhoto(evidence);
